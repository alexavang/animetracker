...
export default function QuickSearch({ onClose }: { onClose(): void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    request<{
      anime: { media: MediaItem[] };
      manga: { media: MediaItem[] };
    }>("https://graphql.anilist.co", SEARCH_BOTH, { search: term })
      .then((data) => {
  ...
  return (
    // 1. Full-screen backdrop covering header + page
    <div
      className="fixed inset-0 bg-black/50 flex items-start justify-center px-4 z-50"
      onClick={onClose}
    >
      {/* 2. Inner panel, pushed down by header height (56px = mt-14) */}
      <div
      ...
      >
        ...
        <div className="w-full max-w-3xl mx-auto">
          <div className="relative">
            ...
            {/* Hint, absolutely positioned under the bar */}
        ...
        {/* 4. Results panels – still inside the mt-14 wrapper */}
        {(animeResults.length > 0 || mangaResults.length > 0) && (
          <div className="grid grid-cols-2 gap-4">
            {/* Anime column */}
            <div>
              <h3 className="mb-2 text-[11px] uppercase tracking-widest text-white/60">
                Anime
              </h3>
